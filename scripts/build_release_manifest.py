...
from videobatch_fast.visual_approval import approval_fingerprint, inspection_manifest_hash, verify_visual_approval
...
def included(path: Path) -> bool:
    ...
    return path.is_file() and not path.is_symlink() and path.name not in EXCLUDE_FILES and not path.name.startswith(".coverage.") and rel.as_posix() not in EXCLUDE_FILES and not any(part in EXCLUDE_PARTS for part in rel.parts) and path.suffix not in EXCLUDE_SUFFIXES
...
def main() -> int:
    ...
    for path in sorted(ROOT.rglob("*")):
        ...
        files.append({"path": path.relative_to(ROOT).as_posix(), "size": len(data), "sha256": sha256_bytes(data), "mode": oct(path.stat().st_mode & 0o777)})
    ...
    payload = {
        ...
        "visual_approval": {
            ...
            "visual_contract_sha256": inspection_manifest_hash(visual) if visual else "",
    ...
    print(f"RELEASE-MANIFEST ERZEUGT · {len(files)} Dateien · {payload['channel']} · visuelle Freigabe {payload['visual_approval']['valid']}")
    return 0
