...
def _validate_one(data: dict[str, object], kind: str, candidate: str, digest: str, now: datetime) -> None:
    name = f"{kind}.json"
    ...
    if data.get("candidate_id") != candidate or data.get("manifest_sha256") != digest:
        raise _blocked(f"{name} gehört nicht zum unveränderten Kandidaten oder Manifest-Hash.", "Die Abnahme für genau diesen Kandidaten und Manifest-Hash durchführen.", "Den belegten Kandidaten separat veröffentlichen.")
...
def validate_evidence(directory: Path, candidate: str, digest: str, *, now: datetime | None = None) -> None:
    current = (now or datetime.now(timezone.utc)).astimezone(timezone.utc)
    for kind in REQUIRED_CHECKS:
        _validate_one(_load(directory / f"{kind}.json"), kind, candidate, digest, current)


def main() -> int:
    parser = argparse.ArgumentParser(description="Prüft externe Stable-Abnahmen strikt lesend.")
