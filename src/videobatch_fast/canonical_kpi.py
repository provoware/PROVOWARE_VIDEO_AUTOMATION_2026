...
def build_kpi_snapshots(
    *,
    ...
    media = _build_media_snapshot(audio_count, media_count, missing_sources, active_tasks, missing_source_names)
    queue = _build_queue_snapshot(
        job_count, completed_jobs, failed_jobs, active_tasks, queue_failure_reasons, retryable_jobs, blocked_jobs
    )
    effects = _build_effects_snapshot(visual_effect, transition, quick_mode, effect_valid, transition_valid)
