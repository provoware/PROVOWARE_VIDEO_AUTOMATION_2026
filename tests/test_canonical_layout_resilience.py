...
def test_canonical_workspace_contains_real_scrollable_dashboard_zones() -> None:
    source = WORKSPACE.read_text(encoding="utf-8")
    ast.parse(source, filename=str(WORKSPACE))
    for token in (
    ...
    assert "self._build_start_page(assistant_body)" in source
...
def test_canonical_window_mixin_is_selected_before_shell_workspace() -> None:
    ...
    assert ui_source.index("CanonicalWindowMixin") < ui_source.index("CanonicalShellWorkspaceMixin")
